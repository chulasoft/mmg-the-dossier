        ...
        return (
          <g key={cell.id} onMouseEnter={() => setHov(cell.id)} onMouseLeave={() => setHov(null)}>
            <polygon points={pts} fill={TERRAIN[cell.terrain].fill}
              ...
              className="lab-hex"
              style={{ transition: "opacity var(--t-base), stroke-width var(--t-fast)", transformBox: "fill-box", transformOrigin: "center", animation: `labTile 0.42s var(--ease-spring) both`, animationDelay: `${((cell.col + cell.row) * 0.02).toFixed(3)}s` }} />
...
function Dispatch({ clue, avgSupport, idx, isHov, onEnter, onLeave }) {
  ...
  return (
    <div className="dispatch" onMouseEnter={onEnter} onMouseLeave={onLeave}
      style={{ animation: `riseIn 0.5s var(--ease-out) both`, animationDelay: `${idx * 0.07}s`, borderLeftColor: fac.color, borderColor: isHov ? fac.color : "#c4b596" }}>
...
.btn{font-family:Georgia,serif;font-size:0.74rem;padding:6px 13px;border:1px solid var(--ink2);background:var(--paper);color:var(--ink);cursor:pointer;border-radius:1px;transition:all var(--t-fast);box-shadow:1px 1px 0 rgba(36,28,18,0.18);}
...
.btn-num{font-family:Georgia,serif;width:28px;height:28px;border:1px solid var(--ink2);background:var(--paper);color:var(--ink2);cursor:pointer;border-radius:1px;font-size:0.82rem;transition:all var(--t-fast);}
.btn-num.on{background:var(--red);color:var(--paper);border-color:var(--red);font-weight:700;}
.btn-tier{font-family:Georgia,serif;font-size:0.68rem;padding:4px 10px;border:1px solid var(--ink2);background:var(--paper);color:var(--ink2);cursor:pointer;border-radius:1px;transition:all var(--t-fast);}
.btn-tier.on{font-weight:700;}
.codex{display:grid;grid-template-columns:repeat(5,1fr);gap:8px;padding:11px 0 4px;animation:riseIn 0.4s var(--ease-out) both;}
...
.news-stamp{letter-spacing:0.04em;font-size:0.62rem;padding:2px 9px;border:1.5px solid;border-radius:2px;animation:stampIn2 0.4s var(--ease-spring) both;}
...
.dispatch{border:1px solid var(--rule);border-left:3px solid;padding:10px 12px;background:var(--paper2);border-radius:1px;transition:transform var(--t-base),box-shadow var(--t-base),border-color var(--t-base);}
...
.bar-fill{height:100%;border-radius:2px;transition:width 0.4s var(--ease-out);}
...
.mx-cell{font-family:Georgia,serif;height:26px;border:1px solid var(--rule);background:var(--paper2);color:var(--ink3);cursor:pointer;border-radius:1px;font-size:0.74rem;transition:all var(--t-fast);}
...
.codex-row{display:flex;align-items:center;gap:8px;text-align:left;font-family:Georgia,serif;cursor:pointer;
  background:var(--paper2);border:1px solid var(--rule);border-left:3px solid;border-radius:1px;padding:7px 9px;transition:all var(--t-fast);}
...
.popup{background:var(--paper);border:1px solid var(--ink);border-top:4px solid;max-width:520px;width:100%;max-height:82vh;overflow-y:auto;padding:20px 22px;box-shadow:4px 6px 0 rgba(36,28,18,0.25);animation:popIn 0.25s var(--ease-spring) both;}
...
.masthead{animation:inkBleed 0.7s var(--ease-out) both;}
.col-map{animation:fadeUpSoft 0.55s 0.05s var(--ease-out) both;}
.col-center{animation:fadeUpSoft 0.55s 0.16s var(--ease-out) both;}
.col-archive{animation:fadeUpSoft 0.55s 0.27s var(--ease-out) both;}
.codex-row{animation:slideFade 0.4s var(--ease-out) both;}
...
.btn,.mx-cell,.codex-row,.archive-item,.leg-item,.pick-btn{transition:all var(--t-fast) var(--ease-out);}
.archive-item{animation:slideFade 0.35s ease both;}
.hex-cell{transition:opacity var(--t-base) var(--ease-out), transform var(--t-base) var(--ease-out);}
...
.gazette :focus-visible{outline:2px solid var(--sepia);outline-offset:2px;}
`;
